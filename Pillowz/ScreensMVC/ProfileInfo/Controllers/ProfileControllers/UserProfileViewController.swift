...
//    let countryField = Field()
    ...
    override func viewDidLoad() {
        ...
        crudVM.options = ["description":"longText"]
        
        if (user_id == nil) {
            user = User.shared
        } else {
            crudVM.isSmallHeaders = true
            loadUser()
        }
...
//        }
        ...
        if (shouldShowUserProfile) {
            ...
            if (user_id == nil) {
                ...
                if let index = fields.index(of: nameField) {
                    fields.remove(at: index)
                    headers.remove(at: index)
                }
                
                if let index = fields.index(of: addressField) {
                    fields.remove(at: index)
                    headers.remove(at: index)
                }
            }
            ...
            fields.insert(reviewField, at: 2)
            headers.insert("", at: 2)
            ...
            if (user.birthday != nil && user.birthday != 0) {
                let dateFormatter = DateFormatter()
            ...
            if (user_id != nil && (user.birthday == nil || user.birthday == 0)) {
            ...
            if (user.isRealtor()) {
                ...
                if (user.realtor!.rtype! == .owner && user_id == nil) {
...
//                }
            ...
            if (user_id == nil && User.shared.realtor?.rtype != .owner) {
                let uploadPhotosField = Field()
                uploadPhotosField.isCustomCellField = true
                uploadPhotosField.customCellObject = ["VC":self, "Space":nil, "certificates":user.realtor!.certificates] as AnyObject
                uploadPhotosField.customCellClassString = "PhotosTableViewCell"
...
//            headers.append("Мои страницы")
        ...
        if (user_id != nil) {
            if (user.shouldLetLeaveReviewAndComplain()) {
                ...
                let reviewField = Field()
                reviewField.type = "ChoiceField"
                reviewField.param_name = "Оставить отзыв"
                reviewField.multiLanguageName = ["ru": "Оставить отзыв", "en":"Оставить отзыв"]
                reviewField.didSelectClosure = { () in
                    let userReviewVC = CreateReviewViewController()
                    userReviewVC.isUser = true
                    userReviewVC.user_id = self.user_id
            ...
            if (user.isRealtor()) {
                let realtorSpacesField = Field()
                realtorSpacesField.isCustomCellField = true
                realtorSpacesField.customCellObject = self
                realtorSpacesField.customCellClassString = "RealtorSpacesTableViewCell"
                fields.append(realtorSpacesField)
                headers.append("")
            }
        }
