...
public class SlidingContainerSliderView: UIScrollView, UIScrollViewDelegate {
    ...
    public var sliderHeight: CGFloat = 38
    ...
    public init(width: CGFloat, titles: [String]) {
        ...
        appearance = SlidingContainerSliderViewAppearance (
            ...
            font: UIFont.init(name: "OpenSans-Regular", size: 13)!,
            selectedFont: UIFont.init(name: "OpenSans-Regular", size: 13)!,
            textColor: UIColor(white: 0.5, alpha: 1.0),
            selectedTextColor: Constants.paletteVioletColor,
            outerPadding: 10,
            innerPadding: 10,
            selectorColor: Constants.paletteVioletColor,
            selectorHeight: 2,
            fixedWidth: false)
    ...
    public func draw() {
        ...
        if appearance.fixedWidth {
            ...
            for title in titles {
                ...
                label.frame.size = CGSize(width: width, height: sliderHeight)
    ...
    public func selectItemAtIndex(_ index: Int) {
        ...
        for i in 0..<self.labels.count {
            ...
            if i == index {
                ...
                self.scrollRectToVisible(CGRect(x: label.frame.origin.x-60, y: self.selector.frame.origin.y, width: label.frame.size.width+120, height: self.appearance.selectorHeight), animated: true)
            } else {
                label.textColor = appearance.textColor
                label.font = appearance.font
                if !appearance.fixedWidth {
